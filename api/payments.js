  ...
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'User-Agent':    'fob-membership-payments-proxy/1.0',
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      let ghData;
      try { ghData = JSON.parse(result.body); }
      catch (e) { return res.status(502).json({ error: 'Invalid JSON from GitHub: ' + e.message }); }
      if (!ghData.content) {
    ...
    if (req.method === 'PUT') {
      ...
      const body    = { message: message || 'mempay: update', content };
      if (sha) body.sha = sha;
      ...
      catch (e) { return res.status(502).json({ error: 'Invalid JSON from GitHub write response' }); }
      return res.status(200).json({ sha: data.content && data.content.sha });
    }
